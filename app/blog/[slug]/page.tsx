    ...
    <main>
      ...
      <section className="relative py-20 bg-snow dark:bg-graphite">
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto px-6 sm:px-10 lg:px-16">
          ...
          <article className="max-w-[820px] prose prose-sm lg:prose-base dark:prose-invert prose-headings:font-bold prose-headings:tracking-tight prose-a:text-accent prose-strong:text-graphite dark:prose-strong:text-snow max-w-none">
            <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]}>
              {post.content}
            </ReactMarkdown>

            <div className="mt-14 p-8 border border-graphite/[0.08] dark:border-white/[0.08] bg-surface/60 dark:bg-surface-dark/60 not-prose">
              <div className="flex items-center gap-3 mb-3">
                <span className="block w-8 h-[2px] bg-led" />
                <span className="text-led text-xs font-semibold tracking-[0.2em] uppercase">Связаться</span>
              </div>
              <p className="text-graphite dark:text-snow font-semibold text-lg mb-2">Нужна консультация по проекту?</p>
              <p className="text-sm text-muted mb-5">Расскажем на конкретном примере, рассчитаем стоимость, ответим на вопросы.</p>
              <RequestButton source="blog-post" className="inline-flex items-center gap-2 px-5 py-3 bg-accent hover:bg-led text-snow text-sm font-semibold transition-colors">
                Получить расчёт
            ...
            <div className="mt-12 pt-8 border-t border-graphite/[0.08] dark:border-white/[0.08] not-prose">
              <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-muted hover:text-accent transition-colors">
                <svg aria-hidden="true" className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <section className="relative py-20 bg-surface dark:bg-surface-dark border-t border-graphite/[0.08] dark:border-white/[0.06]">
          ...
          <div className="relative z-10 max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16">
            <div className="flex items-center gap-3 mb-8">
              <span className="block w-8 h-[2px] bg-led" />
              <span className="text-led text-xs font-semibold tracking-[0.2em] uppercase">Читайте также</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-graphite/[0.08] dark:bg-white/[0.06]">
              {related.map((r) => (
                <Link key={r.id} href={`/blog/${r.slug}`} className="group flex flex-col bg-snow dark:bg-graphite hover:bg-surface/70 dark:hover:bg-surface-dark/70 transition-colors">
                    ...
                    {r.coverImage ? (
                      ...
                      <>
                        ...
                        <div className="absolute inset-0 flex items-center justify-start p-6">
                          <span className="text-snow/[0.05] font-black uppercase tracking-tight leading-none select-none text-5xl">{r.category}</span>
                        </div>
                      </>
                    )}
