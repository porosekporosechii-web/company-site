...
export function Gallery({ works: allWorks }: Props) {
  const works = useMemo(() => sortByPriority(allWorks).slice(0, 6), [allWorks]);
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {works.map((work, idx) => {
              ...
              if (isFeatured) sizeClass = 'md:col-span-2 lg:col-span-2 h-[440px] md:h-[480px] lg:h-[520px]';
              else if (idx === 1) sizeClass = 'h-[380px] md:h-[480px] lg:h-[520px]';
              else if (isWide) sizeClass = 'md:col-span-2 lg:col-span-3 h-[200px] md:h-[230px]';
              else sizeClass = 'h-[300px]';
              ...
              return (
                ...
                >
                  ...
                  {isWide ? (
                    <div aria-hidden="true" className="absolute inset-0 bg-gradient-to-r from-graphite/88 via-graphite/50 to-transparent" />
                  ) : (
                    <div aria-hidden="true" className="absolute inset-0 bg-gradient-to-t from-graphite/92 via-graphite/25 to-graphite/5" />
                  )}
                  <div aria-hidden="true" className="absolute inset-0 bg-graphite/0 group-hover:bg-graphite/20 transition-colors duration-500" />
                  <span className="absolute top-5 left-5 text-led text-[10px] font-bold tracking-[0.3em] uppercase z-10">{num}</span>
                  {hasGallery && (
                  ...
                  {isFeatured && !hasGallery && (
                    <span aria-hidden="true" className="absolute top-5 right-5 w-5 h-5 border-t border-r border-led/35 z-10" />
                  )}
                  {isWide ? (
                    <div className="absolute inset-0 flex items-center px-8 md:px-12 z-10">
                      <div>
                        <span className="block text-white/45 text-[10px] font-semibold uppercase tracking-[0.2em] mb-2">{work.category}</span>
                        <h3 className="font-bold text-snow leading-tight tracking-tight mb-4" style={{ fontSize: 'clamp(1.4rem, 2vw, 2.1rem)' }}>
                          {work.title}
                        </h3>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 z-10">
                      <span className="block text-white/45 text-[10px] font-semibold uppercase tracking-[0.2em] mb-1.5">{work.category}</span>
                      <h3
                        className="font-bold text-snow leading-tight tracking-tight mb-3"
                        style={{ fontSize: isFeatured ? 'clamp(1.6rem, 2.4vw, 2.6rem)' : 'clamp(1.05rem, 1.6vw, 1.45rem)' }}
                      >
                        {work.title}
                      </h3>
