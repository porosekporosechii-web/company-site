...
export function BlogList({ posts, categories }: Props) {
  ...
  const filtered = activeCategory === 'Все статьи'
    ? posts
    : posts.filter((p) => p.category === activeCategory);

  const featured = activeCategory === 'Все статьи'
    ? posts.find((p) => p.featured) ?? filtered[0]
    : filtered[0];

  const grid = activeCategory === 'Все статьи'
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16">
        <AnimateOnScroll>
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-led text-xs font-semibold tracking-[0.2em] uppercase">Материалы</span>
          </div>
        ...
        <div className="mb-12">
          <div className="bg-surface/60 dark:bg-surface-dark/60 border border-graphite/[0.08] dark:border-white/[0.06] p-3 flex gap-2 overflow-x-auto md:flex-wrap">
            {categories.map((cat) => {
              const count = cat === 'Все статьи' ? posts.length : posts.filter((p) => p.category === cat).length;
            ...
            >
              <div className="relative aspect-[4/3] lg:aspect-auto lg:min-h-[460px] overflow-hidden bg-graphite">
                {featured.coverImage ? (
                  ...
                  <>
                    <div aria-hidden="true" className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage: 'linear-gradient(90deg, #fff 1px, transparent 1px), linear-gradient(0deg, #fff 1px, transparent 1px)', backgroundSize: '48px 48px' }} />
                    <div className="absolute inset-0 flex items-end justify-start p-8 overflow-hidden">
                      <span className="text-snow/[0.04] font-black uppercase tracking-tight leading-none select-none text-[7rem] lg:text-[9rem]">{featured.category}</span>
                    </div>
                  </>
                )}
                ...
                <div className="absolute top-5 left-5 z-10 flex items-center gap-2">
                  ...
                  <span className="text-snow text-[10px] font-bold tracking-[0.3em] uppercase">Главное</span>
                </div>
                <span aria-hidden="true" className="absolute top-5 right-5 w-5 h-5 border-t border-r border-led/50 z-10" />
              </div>
              <div className="relative flex flex-col justify-center p-8 sm:p-10 lg:p-12 xl:p-14">
                <div className="flex items-center gap-3 mb-5">
                  <span className="block w-8 h-[2px] bg-led" />
                  <span className="text-led text-[10px] font-semibold tracking-[0.25em] uppercase">{featured.category}</span>
                  <span aria-hidden="true" className="text-muted/50">·</span>
                  <span className="text-muted text-[11px] tracking-wide uppercase">{featured.readTime} чтения</span>
                </div>
                <h3 className="font-bold text-graphite dark:text-snow leading-tight tracking-tight mb-5 group-hover:text-accent transition-colors" style={{ fontSize: 'clamp(1.6rem, 2.6vw, 2.6rem)' }}>
                  {featured.title}
                </h3>
                <p className="text-muted text-base leading-relaxed mb-8 max-w-lg">{featured.excerpt}</p>
        ...
        {grid.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-graphite/[0.08] dark:bg-white/[0.06]">
              {grid.map((post, idx) => (
                <AnimateOnScroll key={post.id} delay={Math.min(idx * 40, 280)} className={`flex ${mdLastColSpan(idx, grid.length)} ${lgLastColSpan(idx, grid.length)}`}>
                  <Link
                    href={`/blog/${post.slug}`}
                  ...
                  >
                    <div className="relative h-[200px] sm:h-[220px] lg:h-[240px] overflow-hidden bg-graphite flex-shrink-0">
                      {post.coverImage ? (
                        ...
                        <>
                          <div aria-hidden="true" className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage: 'linear-gradient(90deg, #fff 1px, transparent 1px), linear-gradient(0deg, #fff 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
                          <div className="absolute inset-0 flex items-end justify-start p-5 overflow-hidden">
                            <span className="text-snow/[0.04] font-black uppercase tracking-tight leading-none select-none text-4xl sm:text-5xl">{post.category}</span>
                          </div>
                        </>
                      )}
                      <div aria-hidden="true" className="absolute inset-0 bg-graphite/0 group-hover:bg-accent/10 transition-colors duration-500" />
                      ...
                      <span aria-hidden="true" className="absolute bottom-4 right-4 w-4 h-4 border-b border-r border-led/30" />
                    </div>
                    <div className="flex flex-col flex-1 p-6 lg:p-7">
                      ...
                      <p className="text-sm text-muted leading-relaxed mb-5 flex-1 line-clamp-3">{post.excerpt}</p>
